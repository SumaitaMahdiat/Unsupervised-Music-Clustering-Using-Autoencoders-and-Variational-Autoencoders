...
# Assume 'song_id' encodes folder/language info (e.g., "german_song1")
def get_label(s):
    s = str(s)
...
# ---------------------------
# Subsample if necessary (Memory issues with Agglo/DBSCAN)
# ---------------------------
MAX_SAMPLES = 5000
if len(X) > MAX_SAMPLES:
    ...
    # We don't strictly need song_ids for the plots, 
    # but we should update train_latent's song_id for the comparison CSV
    subsampled_ids = train_latent['song_id'].values[indices]
else:
    subsampled_ids = train_latent['song_id'].values

# ---------------------------
# Scale Features (Critical for KMeans/DBSCAN)
# ---------------------------
...
# ---------------------------
# Helper: plot clusters
# ---------------------------
def plot_clusters(X_2d, cluster_labels, title, save_path):
    plt.figure(figsize=(10, 7))
    unique_labels = np.unique(cluster_labels)
    
    # Identify non-noise labels for palette mapping
...
# ---------------------------
# 1️⃣ Ground truth (folders/languages)
# ---------------------------
